...
public class LibraryManager {
    ...
    public boolean returnBook(Transaction transaction) {
        if (transaction.getReturnDate() != null) {
            System.out.println("Sudah dikembalikan sebelumnya.");
            return false;
        }

        Book book = getBookByIsbn(transaction.getIsbn());
        if (book != null) {
            transaction.setReturnDate(LocalDate.now());
            transaction.setStatus("Returned");
            book.setQuantity(book.getQuantity() + 1);
            saveAll();
            System.out.println("Berhasil dikembalikan: " + transaction.getIsbn());
            return true;
        }
